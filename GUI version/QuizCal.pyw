...
try:
    ...
    try: 
        subprocess.check_call([sys.executable, "-m", "pip", "install", "tkinter"]); print('tkinker intalled successfully.');
        from tkinter import filedialog
        from tkinter import *
        from tkinter import filedialog
...
def Calculate(Course, SecQs, Resp):
    ...
    return (Course, marks)
def deletepaths():
    os.remove('./csv files/key.txt');os.remove('./csv files/trans.txt')
#################################################################################################################################################################
#running main
def Play(filepaths):
...
# Create a PhotoImage object from the ICO data
window.iconbitmap("./GUI version/csv files/favicon.ico")
# clearing frames...................
def destroy_all_frames(window):
    for widget in window.winfo_children():
        if isinstance(widget, Frame):
